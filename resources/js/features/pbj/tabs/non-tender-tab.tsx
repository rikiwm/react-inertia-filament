...
import { pbjNonTenderService } from '@/Services/pbj-split-service';
import { PbjDetailItem } from '@/Services/pbj-detail-service';

const fmtRupiah = (v: number) =>
    new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(v);

export default function NonTenderTab({ tahun, searchQuery, selectedSatker, initialData }: { tahun: number, searchQuery: string, selectedSatker: string, initialData?: any[] }) {
                ...
                <Table striped className="min-w-full">
                    <Table.Head>
                        <Table.Tr className="bg-rose-50/50 dark:bg-rose-900/10">
                            <Table.Th className="text-[10px] font-semibold uppercase text-rose-400 tracking-widest py-5 px-6">ID Transaksi</Table.Th>
                            <Table.Th className="text-[10px] font-semibold uppercase text-rose-400 tracking-widest py-5 px-6">Detail Paket</Table.Th>
                            <Table.Th className="text-[10px] font-semibold uppercase text-rose-400 tracking-widest py-5 px-6">Satker & Penyedia</Table.Th>
                            <Table.Th className="text-[10px] font-semibold uppercase text-rose-400 tracking-widest py-5 px-6">Status</Table.Th>
                            <Table.Th className="text-[10px] font-semibold uppercase text-rose-400 tracking-widest py-5 px-6 text-right">Nilai</Table.Th>
                        </Table.Tr>
                    </Table.Head>
                    <Table.Body>
                        {loading ? (
                                ...
                                <Table.Tr key={item.order_id} className="hover:bg-slate-50/50 dark:hover:bg-neutral-900/30 transition-colors group cursor-pointer">
                                    <Table.Td className="py-5 px-6">
                                        <div className="flex flex-col gap-1.5">
                                            <span className="inline-flex w-fit items-center px-2 py-0.5 rounded-md text-[9px] font-semibold border uppercase tracking-tight bg-rose-100 text-rose-600 border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800">Non-Tender</span>
                                            <p className="text-xs font-semibold text-slate-900 dark:text-white uppercase mt-1">NT-{item.kd_nontender || '-'}</p>
