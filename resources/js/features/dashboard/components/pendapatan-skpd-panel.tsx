/**
 * @file Components/Dashboard/pendapatan-skpd-panel.tsx
 *
...
import { formatRupiahCompact as fmtR, fmtRupiahFull as fmtRFull } from "@/Lib/formatters";
        ...
        <div className="lg:col-span-1 bg-white dark:bg-neutral-950 rounded-2xl border border-teal-200 dark:border-teal-800 p-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                <div>
                    ...
                    <p className="text-xs text-neutral-500 mt-0.5">Capaian Vs Target Pendapatan — Tahun {tahun}</p>
                </div>
            ...
            {error && (
                <div className="flex items-start gap-3 p-4 rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/20 mb-6">
                    ...
                    <button onClick={retry} className="flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold bg-red-600 hover:bg-red-700 text-white transition-colors">Coba Lagi</button>
                </div>
            )}
            ...
            {data && displayData.length > 0 && (
                <div className="space-y-4 border border-neutral-100 dark:border-neutral-900 p-4 rounded-xl">
                    <ResponsiveContainer width="100%" height={chartHeight}>
                        <BarChart data={displayData} layout="vertical" margin={{ top: 20, right: 16, left: 0, bottom: 0 }} barSize={14}>
                            <CartesianGrid strokeDasharray="2 4" horizontal={true} stroke="#94a3b8" className="dark:[&>line]:stroke-neutral-800" />
                            <XAxis type="number" tickFormatter={fmtR} tick={{ fontSize: 14, fill: "#94a3b8" }} axisLine={false} />
                            ...
                            <Legend iconType="square" iconSize={10} wrapperStyle={{ fontSize: 11, paddingTop: 8 }} formatter={(value) => (<span className="text-neutral-600 dark:text-neutral-400">{value === "realisasi" ? "Capaian" : "Target"}</span>)} />
                            <Bar dataKey="anggaran" fill="#e2e8f0" radius={[0, 8, 8, 0]} name="anggaran" isAnimationActive={shouldAnimate} animationDuration={500} animationEasing="ease-out">
                                {displayData.map((_, i) => (<Cell key={i} className="dark:fill-neutral-800" />))}
                            </Bar>
                            <Bar dataKey="realisasi" fill="#025d58" radius={[0, 8, 8, 0]} name="realisasi" isAnimationActive={shouldAnimate} animationDuration={500} animationEasing="ease-out">
                                {displayData.map((entry, i) => (<Cell key={i} fill={entry.persen >= 75 ? "teal" : entry.persen >= 50 ? "#007470d1" : entry.persen >= 25 ? "#007470a9" : "#00747057"} />))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
...
export const PendapatanSkpdTable = memo(function PendapatanSkpdTable({
    ...
    const handleRowClick = (skpd: any) => {
        const opdSlug = createOpdSlug(skpd.skpd);
        const routeType = "pendapatan";
    ...
    return (
        <div className="lg:col-span-3 space-y-4 bg-white dark:bg-neutral-950 rounded-2xl border border-neutral-200 dark:border-teal-800 p-3">
            <div className="flex items-center justify-between mb-2 mx-auto px-2">
                <h3 className="text-sm font-bold text-neutral-900 dark:text-neutral-100 tracking-tight">Rincian Realisasi Pendapatan SKPD {tahun}</h3>
                    ...
                    <Table.Body>
                        ...
                        {data.map((skpd, i) => (
                            <motion.tr
                                ...
                                key={`${skpd.skpd}-${i}`}
                                onClick={() => handleRowClick(skpd)}
                                className="hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors cursor-pointer"
                            >
                                <Table.Td align="center" className="w-5">{i + 1}</Table.Td>
                                <Table.Td>
                                    <p className="text-neutral-900 uppercase dark:text-teal-400 text-xs mt-0.5 truncate font-semibold max-w-xs sm:max-w-md" title={skpd.label}>{skpd.label}</p>
                                    <p className="text-neutral-400 dark:font-light text-[10px] mt-0.5 truncate max-w-xs sm:max-w-md" title={skpd.skpd}>{skpd.skpd}</p>
                                </Table.Td>
                                <Table.Td align="right" noWrap className="p-0 tabular-nums text-xs">{fmtR(skpd.anggaran)}</Table.Td>
                                <Table.Td align="right" noWrap variant={skpd.persen >= 100 ? "success" : skpd.persen >= 75 ? "warning" : skpd.persen >= 50 ? "info" : "danger"} className="p-0 tabular-nums text-xs font-semibold ">{fmtR(skpd.realisasi)}</Table.Td>
                                <Table.Td align="right" noWrap className="p-0 tabular-nums text-xs text-neutral-500">{fmtR(skpd.sisa)}</Table.Td>
                                <Table.Td align="center" noWrap>
                                    <Table.Badge variant={pctVariant(skpd.persen)}>{skpd.persen.toFixed(1)}%</Table.Badge>
                                </Table.Td>
                            </motion.tr>
                        ))}
                    </Table.Body>
            ...
            <p className="text-neutral-300/90 dark:text-neutral-700 text-right">Sumber: <span className="text-neutral-200 font-semibold text-xs">BADAN PENGELOLAAN KEUANGAN DAN ASET DAERAH KOTA PADANG</span></p>
        </div>
    );
});
