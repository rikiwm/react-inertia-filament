/**
 * @file Services/pkWakoService.ts
 *
 * Service untuk mengambil data Perjanjian Kinerja Wali Kota (PK WAKO).
 * Saat ini menggunakan data dummy sesuai dengan permintaan user.
 */

export interface PkWakoItem {
    id: number;
    sasaran: string;
    indikator: string;
    target: string;
...
const DUMMY_PK_WAKO: PkWakoItem[] = [
    {
        id: 1,
...
export const pkWakoService = {
    /**
     * Mengambil daftar data PK WAKO.
     */
    getPkWakoData: async (): Promise<PkWakoItem[]> => {
        // Simulasi delay API
        return new Promise((resolve) => {
            setTimeout(() => {
                resolve(DUMMY_PK_WAKO);
            }, 500);
        });
    }
};
