...
/**
 * Props untuk SkpdTable komponen.
 *
 * @example
 * ```tsx
 * <SkpdTable
...
/**
 * Format Rupiah untuk display di tabel.
 */
...
/**
 * Membuat slug dari nama OPD untuk URL yang SEO-friendly.
 * Tanpa menggunakan external library.
 */
function createOpdSlug(namaOpd: string): string {
    return namaOpd
        .toLowerCase()
        .trim()
 ...
 *
 * Menampilkan breakdown Belanja Daerah atau Pendapatan Daerah per SKPD
 * dengan kolom nama_opd, pagu/pagu_pad, realisasi/realisasi_pad, dan persentase.
 * User bisa sort kolom dengan klik header, dan setiap nilai Rupiah di-format otomatis.
 */
                    ...
                    <Table.Tr>
                        {headers.map((header) => (
                            ...
                            >
                                <div
                                    className={cn(
                                        "flex items-center gap-2 font-semibold text-neutral-700 dark:text-neutral-300",
                                        header.align === "right" && "justify-end",
                                    )}
                                >
                                    <span>{header.label}</span>
                                    {header.sortable && (
                                        <span className="text-xs opacity-60">{getSortIcon(header.key)}</span>
                                    )}
                                </div>
                            </Table.Th>
                        ))}
                        ...
                        <Table.Tr key={`${item.kd_unit}-${idx}`}
                            ...
                            {type === "belanja" ? (
                                <>
                                    ...
                                    <Table.Td className="text-right text-neutral-700 dark:text-neutral-400 text-sm font-medium">
                                        ...
                                        <div className="flex items-center justify-end gap-2">
                                            <div className="w-12 h-2 bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                                                <div
                                                    ...
                                                    style={{
                                                        width: `${Math.min(item.persentase_anggaran, 100)}%`,
                                                    }}
                                                />
                                            </div>
                                            <span className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 w-12 text-right tabular-nums">
                                                {item.persentase_anggaran.toFixed(1)}%
                                            </span>
                                        </div>
                                    </Table.Td>
                                </>
                                ...
                                <>
                                    ...
                                    <Table.Td className="text-right text-neutral-700 dark:text-neutral-400 text-sm">
                                        {formatRupiah(item.pagu_pad)}
                                    </Table.Td>
                                    <Table.Td className="text-right text-neutral-700 dark:text-neutral-400 text-sm font-medium">
                                        {formatRupiah(item.realisasi_pad)}
                                    </Table.Td>
                                    <Table.Td className="text-right text-neutral-700 dark:text-neutral-400 text-sm">
                                        {formatRupiah(item.sisa_pad)}
                                    </Table.Td>
                                    <Table.Td className="text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <div className="w-12 h-2 bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                                                <div
                                                    className="h-full bg-teal-500 dark:bg-teal-400 rounded-full transition-all"
                                                    style={{
                                                        width: `${Math.min(item.persentase_pad, 100)}%`,
                                                    }}
                                                />
                                            </div>
                                            <span className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 w-12 text-right tabular-nums">
                                                {item.persentase_pad.toFixed(1)}%
                                            </span>
                                        </div>
                                    </Table.Td>
                                </>
