...
import { pbjCatalogService } from '@/Services/pbjSplitService';
                ...
                <Table striped className="min-w-full mx-auto">
                    <Table.Head>
                        <Table.Tr className="bg-teal-50 dark:bg-teal-800">
                            <Table.Th className="text-[10px] font-medium uppercase text-neutral-400 tracking-widest py-4 px-5">Satker</Table.Th>
                            <Table.Th className="text-[10px] font-medium uppercase text-neutral-400 tracking-widest py-3 px-2">Detail Paket</Table.Th>
                            <Table.Th className="text-[10px] font-medium uppercase text-neutral-400 tracking-widest py-3 px-2">Penyedia</Table.Th>
                            <Table.Th className="text-[10px] font-medium uppercase text-neutral-400 tracking-widest py-3 px-2">Status</Table.Th>
                            <Table.Th className="text-[10px] font-medium uppercase text-neutral-400 tracking-widest py-3 px-2 text-right">Nilai</Table.Th>
                        </Table.Tr>
                    </Table.Head>
                    <Table.Body>
                        {loading ? (
                            [...Array(itemsPerPage)].map((_, i) => (
                                <Table.Tr key={i} className="animate-pulse">
                                    <Table.Td colSpan={5} className="py-3 px-6">
                            ...
                            paginatedData.map((item) => (
                                <Table.Tr key={item.order_id} className="hover:bg-slate-50/50 dark:hover:bg-neutral-900/30 transition-colors group cursor-pointer">
                                    ...
                                    <Table.Td className="py-4 px-6">
                                        <div className="space-y-2">
                                            <div className="flex items-start gap-2">
                                                ...
                                                <p className="text-xs font-semibold text-slate-700 dark:text-neutral-300 uppercase leading-tight line-clamp-1">{item.kode_penyedia}</p>
                                            </div>
                                            <div className="flex items-start gap-2">
                                                ...
                                                <span className="inline-flex w-fit items-center px-2 py-0.2 rounded-sm text-[9px] font-medium border uppercase tracking-tight bg-teal-100 text-indigo-600 border-indigo-200 dark:bg-teal-900/30 dark:text-lime-400 dark:border-indigo-800">{item.funding_source}</span>
                                    ...
                                    <Table.Td className="py-4 px-6 text-right tabular-nums">
                                        <p className="text-sm font-semibold text-slate-900 dark:text-white">{fmtRupiah(item.total || item.nilai_kontrak || item.pagu || 0)}</p>
                                    </Table.Td>
                                </Table.Tr>
                            ))
                        ) : (
