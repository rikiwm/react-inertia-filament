...
import { pbjNonTenderService } from '@/Services/pbjSplitService';
import { PbjDetailItem } from '@/Services/pbjDetailService';

const fmtRupiah = (v: number) =>
    new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(v);

export default function NonTenderTab({ tahun, searchQuery, selectedSatker }: { tahun: number, searchQuery: string, selectedSatker: string }) {
    ...
    useEffect(() => {
        let isMounted = true;
        setLoading(true);
        pbjNonTenderService.fetchList(tahun).then(res => {
            if (isMounted) {
                setData(res);
                setLoading(false);
                setCurrentPage(1);
    ...
    const filteredData = useMemo(() => {
        ...
        if (searchQuery) {
            result = result.filter(item =>
                [item.nama_paket, item.nama_satker, item.kd_nontender, item.nama_penyedia]
        ...
        <div className="flex flex-col h-full">
            <div className="overflow-x-auto min-h-[400px]">
                <Table striped className="min-w-full">
                    <Table.Head>
                        <Table.Tr className="bg-rose-50/50 dark:bg-rose-900/10">
                            <Table.Th className="text-[10px] font-black uppercase text-rose-400 tracking-widest py-5 px-6">ID Transaksi</Table.Th>
                            <Table.Th className="text-[10px] font-black uppercase text-rose-400 tracking-widest py-5 px-6">Detail Paket</Table.Th>
                            <Table.Th className="text-[10px] font-black uppercase text-rose-400 tracking-widest py-5 px-6">Satker & Penyedia</Table.Th>
                            <Table.Th className="text-[10px] font-black uppercase text-rose-400 tracking-widest py-5 px-6">Status</Table.Th>
                            <Table.Th className="text-[10px] font-black uppercase text-rose-400 tracking-widest py-5 px-6 text-right">Nilai</Table.Th>
                        </Table.Tr>
                    </Table.Head>
                    <Table.Body>
                        {loading ? (
                                ...
                                <Table.Tr key={item.order_id} className="hover:bg-slate-50/50 dark:hover:bg-neutral-900/30 transition-colors group cursor-pointer">
                                    <Table.Td className="py-5 px-6">
                                        <div className="flex flex-col gap-1.5">
                                            <span className="inline-flex w-fit items-center px-2 py-0.5 rounded-md text-[9px] font-black border uppercase tracking-tight bg-rose-100 text-rose-600 border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800">Non-Tender</span>
                                            <p className="text-xs font-black text-slate-900 dark:text-white uppercase mt-1">NT-{item.kd_nontender || '-'}</p>
                                            <p className="text-[10px] text-slate-400 font-mono tracking-tight">RUP: {item.kd_rup || '-'}</p>
                                        </div>
                                    </Table.Td>
                                    <Table.Td className="py-5 px-6 max-w-xs">
                                        <p className="text-sm font-semibold text-slate-800 dark:text-neutral-200 line-clamp-2 group-hover:text-rose-600 transition-colors">{item.nama_paket}</p>
                                        <div className="flex items-center gap-1.5 mt-2 opacity-60">
                                            <Calendar className="w-3 h-3" />
                                            <span className="text-[10px] font-medium">{new Date(item.tanggal_buat || '2024-01-01').toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                                        </div>
                                    </Table.Td>
                                    <Table.Td className="py-5 px-6">
                                    ...
                                    <Table.Td className="py-5 px-6 text-right tabular-nums">
                                        <p className="text-sm font-semibold text-slate-900 dark:text-white">{fmtRupiah(item.nilai_kontrak || item.nilai_negosiasi || item.pagu || 0)}</p>
                                    </Table.Td>
                                </Table.Tr>
                            ))
                        ) : (
